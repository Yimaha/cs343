...
void __insertdone__( unsigned int ticket ) { \
	if ( ticket != __insertdoneC__ ) { cerr << "BARGING ERROR!! producer over producer" << endl; ABORT } \
	if ( __next__ == 2 ) { cerr << "BARGING ERROR!! producer over consumer" << endl; ABORT } \
	__next__ = 0; \
	__insertdoneC__ += 1; \
} \
...
void __removedone__( unsigned int ticket ) { \
	if ( ticket != __removedoneC__ ) { cerr << "BARGING ERROR!! consumer over consumer" << endl; ABORT } \
	if ( __next__ == 1 ) { cerr << "BARGING ERROR!! consumer over producer" << endl; ABORT } \
	__next__ = 0; \
	__removedoneC__ += 1; \
} \
